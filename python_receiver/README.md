...
While on Linux, it is important. Asyncio will return immediately when looking at the file descriptor, and it is not guaranteed with the `read()` method to obtain enough samples. Yes, `read(1024)` in this way can provide low latency, but then the signal processing part of the code is prone to bugs (you can fix it using the `x.size` variable instead of `bufsize`, but still, it isn't the best solution).
...
* [Previous project](https://github.com/TIT8/shelly_button_esp32_arduino/tree/master/speech_recognition)
